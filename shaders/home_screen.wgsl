...
//!
//! Procedurally renders the console library carousel, pitch-black OLED background,
...
fn get_glyph_16seg(c: u32) -> u32 {
    ...
    if (c == 73u) { return 3123u;  } // I (Serif top + bottom + center spine)
...
fn draw_vector_char(p: vec2<f32>, raw_c: u32, sz: vec2<f32>, stroke_w: f32) -> f32 {
    // Automatically map lowercase a..z (97..122) to uppercase A..Z (65..90)
    let c = select(raw_c, raw_c - 32u, raw_c >= 97u && raw_c <= 122u);
    ...
    // Punctuation support
    if (c == 95u) { // '_'
        return sd_segment(p, bl, br) - stroke_w;
    }
    if (c == 46u) { // '.'
        return sd_circle(p - vec2<f32>(0.0, h * 0.75), stroke_w * 1.6) - stroke_w;
    }

    // Crisp, handcrafted vector shapes for complex letters
    if (c == 68u) { // 'D' (Curved spine, NO vertical center line)
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tl, vec2<f32>(w * 0.35, -h)));
        d = min(d, sd_segment(p, vec2<f32>(w * 0.35, -h), vec2<f32>(w, -h * 0.35)));
        d = min(d, sd_segment(p, vec2<f32>(w, -h * 0.35), vec2<f32>(w, h * 0.35)));
        d = min(d, sd_segment(p, vec2<f32>(w, h * 0.35), vec2<f32>(w * 0.35, h)));
        d = min(d, sd_segment(p, vec2<f32>(w * 0.35, h), bl));
        return d - stroke_w;
    }
    if (c == 66u) { // 'B' (Two distinct lobes with waist, NO interior cross)
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tl, vec2<f32>(w * 0.45, -h)));
        d = min(d, sd_segment(p, vec2<f32>(w * 0.45, -h), mr));
        d = min(d, sd_segment(p, ml, mr));
        d = min(d, sd_segment(p, mr, vec2<f32>(w * 0.45, h)));
        d = min(d, sd_segment(p, vec2<f32>(w * 0.45, h), bl));
        return d - stroke_w;
    }
    if (c == 82u) { // 'R' (Straight spine, upper loop, angled right leg)
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tl, vec2<f32>(w * 0.50, -h)));
        d = min(d, sd_segment(p, vec2<f32>(w * 0.50, -h), mr));
        d = min(d, sd_segment(p, ml, mr));
        d = min(d, sd_segment(p, vec2<f32>(-w * 0.05, 0.0), br));
        return d - stroke_w;
    }
    if (c == 86u) { // 'V' (Clean symmetrical slants meeting at bottom center)
        d = min(d, sd_segment(p, tl, bc));
        d = min(d, sd_segment(p, tr, bc));
        return d - stroke_w;
    }
    if (c == 77u) { // 'M'
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tr, br));
        d = min(d, sd_segment(p, tl, mc));
        d = min(d, sd_segment(p, tr, mc));
        return d - stroke_w;
    }
    if (c == 78u) { // 'N'
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tr, br));
        d = min(d, sd_segment(p, tl, br));
        return d - stroke_w;
    }
    if (c == 87u) { // 'W'
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, tr, br));
        d = min(d, sd_segment(p, bl, mc));
        d = min(d, sd_segment(p, br, mc));
        return d - stroke_w;
    }
    if (c == 75u) { // 'K'
        d = min(d, sd_segment(p, tl, bl));
        d = min(d, sd_segment(p, ml, tr));
        d = min(d, sd_segment(p, ml, br));
        return d - stroke_w;
    }
    if (c == 88u) { // 'X'
        d = min(d, sd_segment(p, tl, br));
        d = min(d, sd_segment(p, tr, bl));
        return d - stroke_w;
    }
    if (c == 90u) { // 'Z'
        d = min(d, sd_segment(p, tl, tr));
        d = min(d, sd_segment(p, tr, bl));
        d = min(d, sd_segment(p, bl, br));
        return d - stroke_w;
    }
...
fn draw_vector_string_16(
    ...
    if (idx < 4u) {
        if (idx == 0u) { code = v0.x; }
        else if (idx == 1u) { code = v0.y; }
        else if (idx == 2u) { code = v0.z; }
        else { code = v0.w; }
    } else if (idx < 8u) {
        let sub = idx - 4u;
        if (sub == 0u) { code = v1.x; }
        else if (sub == 1u) { code = v1.y; }
        else if (sub == 2u) { code = v1.z; }
        else { code = v1.w; }
    } else if (idx < 12u) {
        let sub = idx - 8u;
        if (sub == 0u) { code = v2.x; }
        else if (sub == 1u) { code = v2.y; }
        else if (sub == 2u) { code = v2.z; }
        else { code = v2.w; }
    } else if (idx < 16u) {
        let sub = idx - 12u;
        if (sub == 0u) { code = v3.x; }
        else if (sub == 1u) { code = v3.y; }
        else if (sub == 2u) { code = v3.z; }
        else { code = v3.w; }
    }
    if (code == 0u || code == 32u) {
        return 1e5;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // 1. Deep OLED Pitch Black Backdrop (Zero banding rings, maximum contrast)
    var final_color = vec4<f32>(0.0, 0.0, 0.0, 1.0);

    // 2. Top Header: Title, Subtitle, and Divider
    let hdr_y = 0.10;
    let title_sz = vec2<f32>(0.012, 0.020);
    let title_spacing = 0.016;
    let title_stroke = 0.0016;
    // Exactly centered: start_x = 0.50 * aspect - (10 - 1) * title_spacing * 0.5
    let title_start_x = 0.50 * aspect - 0.072;

    // "PIXELDRIVE"
    let logo_d = draw_vector_string_16(
        p - vec2<f32>(0.0, hdr_y),
        title_start_x,
        title_sz,
        title_spacing,
        title_stroke,
        vec4<u32>(80u, 73u, 88u, 69u), // P, I, X, E
        vec4<u32>(76u, 68u, 82u, 73u), // L, D, R, I
        vec4<u32>(86u, 69u, 0u, 0u),   // V, E
    ...
    // "LIBRARY" subtitle
    let sub_y = 0.142;
    let sub_sz = vec2<f32>(0.008, 0.013);
    let sub_spacing = 0.013;
    let sub_stroke = 0.0013;
    // Exactly centered: start_x = 0.50 * aspect - (7 - 1) * sub_spacing * 0.5
    let sub_start_x = 0.50 * aspect - 0.039;

    let sub_d = draw_vector_string_16(
        p - vec2<f32>(0.0, sub_y),
        sub_start_x,
        sub_sz,
        sub_spacing,
        sub_stroke,
        vec4<u32>(76u, 73u, 66u, 82u), // L, I, B, R
        vec4<u32>(65u, 82u, 89u, 0u),  // A, R, Y
        vec4<u32>(0u, 0u, 0u, 0u),
        vec4<u32>(0u, 0u, 0u, 0u),
        7u
    );
    ...
    // Horizontal header divider rule (balanced to match card width 0.285)
    let div_p = p - vec2<f32>(0.50 * aspect, 0.170);
    let div_half_w = 0.285;
    let div_d = sd_segment(div_p, vec2<f32>(-div_half_w, 0.0), vec2<f32>(div_half_w, 0.0)) - 0.0008;
    ...
    if (uniforms.selected_index > 0u) {
        ...
        let prev_fill = vec4<f32>(0.04, 0.05, 0.07, 0.50 * smoothstep(aa, -aa, prev_d));
        let prev_rim = vec4<f32>(0.20, 0.26, 0.34, 0.45 * smoothstep(aa, -aa, abs(prev_d + 0.0015) - 0.0015));
    ...
    if (uniforms.selected_index < uniforms.total_tiles - 1u) {
        ...
        let next_fill = vec4<f32>(0.04, 0.05, 0.07, 0.50 * smoothstep(aa, -aa, next_d));
        let next_rim = vec4<f32>(0.20, 0.26, 0.34, 0.45 * smoothstep(aa, -aa, abs(next_d + 0.0015) - 0.0015));
    ...
    let shadow_alpha = 0.55 * smoothstep(0.045, -0.005, shadow_d);
    ...
    if (card_d < 0.02) {
        ...
        var interior_col = vec4<f32>(0.03, 0.04, 0.06, 0.98 * card_alpha);
        ...
        if (uniforms.is_add_game_selected != 0u) {
            ...
            let add_txt_d = draw_vector_string_16(
                card_p - vec2<f32>(0.0, 0.055),
                -0.049,
                vec2<f32>(0.009, 0.015),
                0.014,
                0.0014,
                vec4<u32>(65u, 68u, 68u, 32u), // A, D, D, ' '
                vec4<u32>(71u, 65u, 77u, 69u), // G, A, M, E
                vec4<u32>(0u, 0u, 0u, 0u),
                vec4<u32>(0u, 0u, 0u, 0u),
                8u
            );
    ...
    let spacing = 0.015;
    let stroke_w = 0.0016;
    let title_len = min(uniforms.title_len, 16u);
    var label_start_x = 0.50 * aspect;
    if (title_len > 1u) {
        label_start_x = 0.50 * aspect - f32(title_len - 1u) * spacing * 0.5;
    }

    let title_p = p - vec2<f32>(0.0, label_y);
    let title_d = draw_vector_string_16(
        title_p,
        label_start_x,
        char_sz,
        spacing,
        stroke_w,
        uniforms.title_chars_0,
        uniforms.title_chars_1,
        uniforms.title_chars_2,
        uniforms.title_chars_3,
        title_len
    );
    let title_col = vec4<f32>(1.0, 1.0, 1.0, 0.95 * smoothstep(aa, -aa, title_d));
    final_color = blend_over(title_col, final_color);

    // 5. Navigation Dots (Pagination, elevated to 0.78 for clean visual balance)
    let dot_y = 0.78;
    let total_dots = min(uniforms.total_tiles, 10u);
    let dot_spacing = 0.022;
    var dot_start_x = 0.50 * aspect;
    if (total_dots > 1u) {
        dot_start_x = 0.50 * aspect - f32(total_dots - 1u) * dot_spacing * 0.5;
    }
    ...
    for (var i = 0u; i < total_dots; i = i + 1u) {
        ...
        let dot_color = select(vec4<f32>(0.25, 0.30, 0.38, 0.50), vec4<f32>(0.32, 0.85, 0.78, 0.95), is_active);
